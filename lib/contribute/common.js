...
export async function handleCommonDirAndFiles(dir, options) {
  ...
  if (options.features.includes('Linter / Formatter')) {
    ...
    if (options.jsType === 'typescript') {
      fs.copyFileSync(
        path.resolve(__dirname, '../templates/src/common/tslint.json'),
        path.resolve(dir, './tslint.json'),
      );
      fs.copyFileSync(
        path.resolve(__dirname, '../templates/src/common/tsconfig.json'),
        path.resolve(dir, './tsconfig.json'),
      );
    } else {
      fs.copyFileSync(
        path.resolve(__dirname, '../templates/src/common/.eslintrc'),
        path.resolve(dir, './.eslintrc'),
      );
      fs.copyFileSync(
        path.resolve(__dirname, '../templates/src/common/.eslintignore'),
